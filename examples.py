...
def main():
    ...
    try:
        ...
        print(
            f"Found {transaction_response.total} total transactions (showing first 10):"
        )

        for transaction in transaction_response.transactions:
            print(
                f"  - {transaction.createdAt.strftime('%Y-%m-%d')}: "
        ...
        pending_transactions = client.get_transactions(
            account_id=first_account.id, status="pending", limit=5
        )

        if pending_transactions.transactions:
            print(
                f"Found {len(pending_transactions.transactions)} pending transactions:"
            )
            for transaction in pending_transactions.transactions:
                print(
                    f"  - ${transaction.amount:.2f} to {transaction.counterpartyName}"
                )
        else:
        ...
        if transaction_response.transactions:
            ...
            specific_transaction = client.get_transaction_by_id(
                account_id=first_account.id, transaction_id=first_transaction.id
            )
            ...
            if specific_transaction:
                ...
                if specific_transaction.attachments:
                    ...
                    for attachment in specific_transaction.attachments:
                        print(
                            f"    - {attachment.fileName} ({attachment.attachmentType})"
                        )
